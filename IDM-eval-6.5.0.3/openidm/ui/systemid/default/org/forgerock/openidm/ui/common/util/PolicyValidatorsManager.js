...
define(["jquery", "underscore", "form2js", "org/forgerock/commons/ui/common/main/ValidatorsManager", "org/forgerock/openidm/ui/common/delegates/PolicyDelegate"], function ($, _, form2js, ValidatorsManager, PolicyDelegate) {
    ...
    */
    ...
    obj.getTranslationForPolicyFailure = function (policyFailure) {
        ...
        } else if (_.has(policyFailure, "params.invalidArrayItems")) {
            policyFailure.params.invalidArrayItems = obj.getInvalidArrayItemsText(policyFailure.params.invalidArrayItems);
        }

        return $.t("common.form.validation." + policyFailure.policyRequirement, policyFailure.params);
    };

    obj.getInvalidArrayItemsText = function (invalidArrayItems) {
        var invalidArrayItemsTextArray = [];

        _.each(invalidArrayItems, function (invalidArrayItem) {
            invalidArrayItemsTextArray.push(obj.failedPolicyRequirementObjectsToStrings(invalidArrayItem.failedPolicyRequirements));
        });

        return invalidArrayItemsTextArray.join(', ');
    };
