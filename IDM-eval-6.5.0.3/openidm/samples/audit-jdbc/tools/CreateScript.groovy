...
switch ( objectClass ) {
    ...
    case auditauthentication:
        sql.execute("INSERT INTO auditauthentication " +
                "(objectid, transactionid, activitydate, userid, eventname, provider, method, result, principals, context, " +
                "entries, trackingids" +
                ") values (?,?,?,?,?,?,?,?,?,?,?,?)",
                [
                    id,
                    ...
                    attributeMap.get("userid")?.getValue()?.get(0),
                    attributeMap.get("eventname")?.getValue()?.get(0),
                    attributeMap.get("provider")?.getValue()?.get(0),
                    attributeMap.get("method")?.getValue()?.get(0),
    ...
    case auditactivity:
        sql.execute("INSERT INTO auditactivity (" +
                ...
                "subjectrev, passwordchanged, message, provider, context, status" +
                ") values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                [
                    id, // objectid
                    attributeMap.get("activitydate")?.getValue()?.get(0),
                    ...
                    attributeMap.get("provider")?.getValue()?.get(0),
                    attributeMap.get("context")?.getValue()?.get(0),
